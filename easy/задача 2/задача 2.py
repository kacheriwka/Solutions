...
for x in range(123456,222222):  # Перебираем числа в нашем диапазоне.
    ...
    if d%100==0: #т.к я не могу делить на 0, я пропускаю числа, которые в конце имеют 00
        continue
    if x%(d%100)==0: #если наше число делится на 2 последние цифры числа m, к количеству прибавляю единичку
        k+=1
        print(x,k)
